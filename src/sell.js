...
/* ══════════════════════════════════════════════════════════════
   MÓDULO: SELL  v2.0
   FIX BUG 7: Sell ya no es una card separada debajo del swap.
   Ahora es un panel dentro de la misma swap card, activado
   mediante una pestaña [Swap | Sell] al estilo PancakeSwap.

   La tab "Sell" está deshabilitada cuando buyback está inactivo
   y muestra un tooltip explicativo. Cuando está activo, se activa
   la tab y el usuario puede cambiar a ella para vender.
══════════════════════════════════════════════════════════════ */
const SELL = {
  _inProgress:    false,
  ...
  /*
   * checkBuybackStatus(): Consulta el contrato para saber si
   * buyback está activo. Actualiza el estado del tab "Sell".
   */
  ...
  /*
   * _renderStatus(active, bnbLiq): Actualiza el tab de sell.
   * FIX BUG 7: En lugar de mostrar/ocultar una card separada,
   * habilita/deshabilita el tab "Sell" dentro de la swap card.
   */
  _renderStatus(active, bnbLiq) {
    const sellTab  = document.getElementById('tabSell');
    const sellBody = document.getElementById('panelSell');
    const badge    = document.getElementById('sellTabBadge');

    if (!sellTab) return;

    if (active) {
      sellTab.disabled = false;
      sellTab.classList.remove('tab-disabled');
      sellTab.title = '';
      if (badge) { badge.textContent = ''; badge.className = 'sell-tab-badge-on'; }

      // Actualizar símbolo token en el panel sell
      ...
      // Inactivo: tab visible pero deshabilitado con tooltip
      sellTab.disabled = false; // seguimos permitiendo clic para mostrar mensaje
      ...
      // Si el panel sell estaba visible, volver a swap
  ...
  /*
   * activateTab(): Cambia al panel de venta.
   * Se llama cuando el usuario hace clic en el tab "Sell".
   */
  activateTab() {
